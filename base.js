...
for (let col = 0; col < parsedArray[0].length; col++) {
    ...
    for (const row of parsedArray) {
        if (row[col] !== null && row[col] !== 'empty') {
            uniqueValues.add(row[col]); // Add non-null value to the Set
        }
    }
    // Convert Set to Array, sort it alphabetically, and push to result array
    uniqueParsedArray.push(Array.from(uniqueValues).sort());
}
    ...
    for(let i = 0; i<uniqueParsedArray[4].length & i<checkboxMax; i++){
        ...
        label.innerHTML += `
            <input type="checkbox" name="options" onclick=dispOne("${content.replaceAll(" ", "")}") value="${i}">${content}`;
    ...
    departureContainer.appendChild(nextButtonDepart);
     ...
     for(let i = 0; i<uniqueParsedArray[3].length & i<checkboxMax; i++){
        ...
        const label = document.createElement('label');
        label.innerHTML += `
            <input type="checkbox" name="options" onclick=dispTwo("${content.replaceAll(" ", "")}") value="${i}">${content}`;
    ...
    arrivalContainer.appendChild(nextButtonArrival);
     ...
     for(let i = 0; i<uniqueParsedArray[2].length & i<checkboxMax; i++){
        ...
        const label = document.createElement('label');
        label.innerHTML += `
            <input type="checkbox" name="options" onclick=dispFour("${content.replaceAll(" ", "")}") value="${i}">${content}`;
    ...
    nextButtonAirline.addEventListener('click', () => {//event listener to make departures disappear
     ...
     for(let i = 0; i<uniqueParsedArray[0].length & i<checkboxMax; i++){
        ...
        const label = document.createElement('label');
        label.innerHTML += `
            <input type="checkbox" name="options" onclick=dispThree("${content.replaceAll(" ", "")}") value="${i}">${content}`;
...
function generateFlights(){
    ...
    for(let i = 0; i < parsedArray.length; i++){
        ...
        let score = 0;
        for(let j = 0; j < departs.length; j++){
            if(departure !== null && departure.replaceAll(" ", "") === departs[j]){
                console.log("Going into if statement");
                score += 5;
            }
        }
        for(let k = 0; k < arrivals.length; k++){
            if(arrival !== null && arrival.replaceAll(" ", "") === arrivals[k]){
                score += 5;
            }
        }
        for(let l = 0; l < dates.length; l++){
            if(arrival !== null && date.replaceAll(" ", "") === dates[l]){
                score += 1;
            }
        }
        for(let m = 0; m < airlines.length; m++){
            if(airline !== null && airline.replaceAll(" ", "") === airlines[m]){
                score += 1;
            }
        }
        scores.push(score);
        console.log("Score for element " + i + ": " + score);
    ...
    if(firstScore <= 7 && secondScore <= 7 && thirdScore <= 7){
        flightDiv.innerHTML += "There are no flights which have that departure and arrival. Please try again with broader answers" + "<br>";
    }
    else{
        flightDiv.innerHTML += "Here are the flight(s) found which best meet your specifications: <br>";
        if(firstScore > 7){
            flightDiv.innerHTML += "1. Flight number " + parsedArray[firstPos][1] + " leaves from " + parsedArray[firstPos][4] + " and arrives at " + parsedArray[firstPos][3] + " on " + parsedArray[firstPos][0] + " traveling by " + parsedArray[firstPos][2] + "<br>";
        }
        if(secondScore > 7){
            flightDiv.innerHTML += "2. Flight number " + parsedArray[secondPos][1] + " leaves from " + parsedArray[secondPos][4] + " and arrives at " + parsedArray[secondPos][3] + " on " + parsedArray[secondPos][0] + " traveling by " + parsedArray[secondPos][2] + "<br>";
        }
        if(thirdScore > 7){
            flightDiv.innerHTML += "3. Flight number " + parsedArray[thirdPos][1] + " leaves from " + parsedArray[thirdPos][4] + " and arrives at " + parsedArray[thirdPos][3] + " on " + parsedArray[thirdPos][0] + " traveling by " + parsedArray[thirdPos][2] + "<br>";
        }
    }
